...
# Ejecutar tests de regresión de la extensión qmd-ptm-ty-slides
...
#
# Renderiza cada test-*.qmd a qmd-ptm-ty-slides-typst (PDF).
...
extension_dst <- file.path(test_dir, "_extensions", "qmd-ptm-ty-slides")

# Sincronizar extensión desde la fuente principal antes de testear
extension_src <- "_extensions/qmd-ptm-ty-slides"
cat("Sincronizando extensión...\n")
...
for (f in qmd_files) {
  for (fmt in c("qmd-ptm-ty-slides-typst")) {
    out <- basename(f)
